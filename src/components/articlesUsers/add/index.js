...
export default function AddArticleUser() {

    const navigate = useNavigate();
    const [state, setState] = useState({ title: "", explanation: "" })
    const setValueInput = (name, event) => { setState(prevState => ({ ...prevState, [name]: event.target.value })) };

    const cancelHandler = () => {
        setState({ title: "", explanation: "" });
        navigate("/dashboard/articlesUsers");
    };

    const { loading, confirm } = useConfirm(state);
    ...
    return (
        <DashboardLayout>
            ...
            <div className="flex flex-row justify-between">
                <h1 className="text-3xl">افزودن مقاله</h1>
                <Header/>
                </div>
                <hr />
                    ...
                    <div className="grid grid-cols-1 gap-y-6 md:grid-cols-6 sm:gap-x-6">
                        ...
                        <div className="col-span-6">
                            ...
                            <textarea type="text" onChange={setValueInput.bind(null, "explanation")} value={state.explanation}
                                placeholder="توضیحات مقاله" className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 min-h-[130px]" name="explanation" />
                        </div>
                    </div>
                    <div>
                        <div className="pt-8 flex justify-end">
                            ...
                            <button type="submit" onClick={confirmHandler}
                                className="ltr:ml-3 rtl:mr-3 inline-flex justify-center py-2 px-4 border border-transparent shadow rounded text-white bg-violet-500 font-bold drop-shadow hover:bg-violet-600 active:bg-violet-700 focus:ring focus:ring-violet-300"
                            >اضافه کردن  {loading ? <Loading /> : ""}</button>
                        </div>
                    </div>
                </form>
            </div>
